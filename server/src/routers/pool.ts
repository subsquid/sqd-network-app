...
import { portalPoolAbi } from '../../../abi/portalPool.js';
import { portalPoolFactoryAbi } from '../../../abi/portalPoolFactory.js';
import { portalRegistryAbi } from '../../../abi/portalRegistry.js';
...
export const poolRouter = router({
  get: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const rewardToken = rewardTokenAddress
        ? tokens.find(
            t => t.address.toLowerCase() === (rewardTokenAddress as string).toLowerCase(),
          )
        : undefined;
      ...
      const totalRewardsToppedUp = BigNumber(
        poolIndexedData.poolById?.totalRewardsToppedUp ?? '0',
      )
        .shiftedBy(-rewardToken.decimals)
        .toFixed();
  ...
  apyTimeseries: publicProcedure
    .input(z.object({ poolId: z.string().toLowerCase(), from: z.date({coerce: true}), to: z.date({coerce: true}) }))
    .query(async ({ input }) => {
    ...
    .query(async ({ input }) => {
      ...
      return {
        liquidityEvents: data.liquidityEvents.map(e => ({
          ...e,
          amount: BigNumber(e.amount).shiftedBy(-(lptToken?.decimals ?? SQD_DECIMALS)).toFixed(),
        })),
    ...
    .query(async ({ input }) => {
      ...
      return {
        topUps: data.topUps.map(t => ({
          ...t,
          amount: BigNumber(t.amount).shiftedBy(-(rewardToken?.decimals ?? 6)).toFixed(),
        })),
      ...
      for (let i = 0; i < ticketIds.length; i++) {
        ...
        const exitTicket =
          exitTicketResult.status === 'success'
            ? (exitTicketResult.result as { endPosition: bigint; amount: bigint; withdrawn: boolean })
            : null;
        const queueStatus =
